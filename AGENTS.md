...
### MSWによるSupabase認証のモック

ローカル開発環境やテストで`apps/admin`のような認証が必要なアプリケーションを動作させるために、MSW（Mock Service Worker）を利用してSupabaseの認証フローをシミュレートしています。

#### ログイン状態のシミュレート

MSWは、`MSW_SUPABASE_AUTHENTICATED`という環境変数をチェックして、認証状態を切り替えることができます。

*   **`MSW_SUPABASE_AUTHENTICATED=true`**:
    *   この環境変数を設定すると、アプリケーションは常に認証済みの状態として扱われます。
    *   これにより、手動でのログイン操作を省略して、認証が必要なページに直接アクセスできます。
    *   主に自動テストや、UIコンポーネントの開発に集中したい場合に便利です。

*   **`MSW_SUPABASE_AUTHENTICATED=false` または未設定**:
    *   アプリケーションは未認証の状態から開始されます。
    *   `apps/admin`のログインページ（`/login`）で以下の認証情報を使用して、手動でログインフローをテストすることができます。
        *   **メールアドレス**: `admin@example.com`
        *   **パスワード**: `password123`

この機能により、開発者は認証状態を柔軟に制御し、効率的に開発とテストを進めることができます。
